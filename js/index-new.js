...
function initPageTransitions() {
  ...
  function initSmoothScroll(container) {
    ...
    try {
      ...
      // Add error handling for scroll updates
      window.onresize = () => {
        try {
          scroll.update();
        } catch (error) {
      ...
      ScrollTrigger.scrollerProxy('[data-scroll-container]', {
        scrollTop(value) {
          try {
            ...
            console.warn('Scroll proxy error:', error);
            return 0;
          }
        },
      ...
      ScrollTrigger.addEventListener('refresh', () => {
        try {
          ...
          console.warn('Scroll refresh error:', error);
        }
      });
...
// Add safe scroll control functions at the top of the file
function safeScrollStart() {
  try {
    if (scroll && typeof scroll.start === 'function') {
      scroll.start();
      console.log('Scroll started safely');
    }
  } catch (error) {
    console.warn('Error starting scroll:', error);
  }
}

function safeScrollStop() {
  try {
    if (scroll && typeof scroll.stop === 'function') {
      scroll.stop();
      console.log('Scroll stopped safely');
    }
  } catch (error) {
    console.warn('Error stopping scroll:', error);
  }
}

// Add scroll recovery function
function recoverScroll() {
  try {
    if (scroll && typeof scroll.update === 'function') {
      scroll.update();
      console.log('Scroll recovered');
    }
  } catch (error) {
    console.warn('Scroll recovery failed:', error);
    ...
    setTimeout(() => {
      try {
        ...
        console.error('Scroll reinitialization failed:', reinitError);
      }
    }, 100);
  }
}

// Add scroll recovery on window focus
window.addEventListener('focus', recoverScroll);

// Add scroll recovery on visibility change
...
function debugScroll() {
  ...
  console.log('Scroll object exists:', !!scroll);
  console.log('Scroll container exists:', !!scrollContainer);
  console.log('Main element exists:', !!mainElement);
  console.log('Navigation active:', mainElement?.classList.contains('nav-active'));
  console.log('Loading container visible:', document.querySelector('.loading-container')?.style.display !== 'none');
  console.log('Scroll container transform:', scrollContainer?.style.transform);
  console.log('Main overflow:', mainElement?.style.overflow);
...
// Auto-debug when scrolling issues might occur
setInterval(() => {
  const scrollContainer = document.querySelector('[data-scroll-container]');
  if (scrollContainer && !scrollContainer.style.transform && !document.querySelector('main').classList.contains('nav-active')) {
    console.warn('Potential scroll issue detected - scroll container has no transform');
    debugScroll();
  }
}, 5000);
